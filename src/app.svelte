...
<style>
  ...
  *{ background:none; }
  ...
  .notifications-container{ position:fixed; top:2vh; right:2vh; display:flex; flex-direction:column; gap:.8vh; width:min(90vw,32vh); z-index:999999; pointer-events:none; }
  .notification{ position:relative; pointer-events:auto; width:100%; min-height:5.5vh; overflow:hidden; transition:all .25s cubic-bezier(.34,1.56,.64,1); }
  .notification-bg{ position:absolute; inset:0; background:var(--bg-900); backdrop-filter:blur(16px) saturate(160%); -webkit-backdrop-filter:blur(16px) saturate(160%); border-radius:.8vh; box-shadow:0 .6vh 2.4vh rgba(0,0,0,.4), 0 .3vh .8vh rgba(0,0,0,.25), inset 0 .1vh .15vh rgba(255,255,255,.03); }
  .notification-border{ position:absolute; inset:0; border-radius:.8vh; padding:.12vh; background:linear-gradient(135deg, var(--notification-accent,var(--accent)) 0%, transparent 100%); -webkit-mask:linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite:xor; mask-composite:exclude; opacity:.5; transition:opacity .25s ease; }
  .notification:hover .notification-border{ opacity:.85; }
  .notification-content{ position:relative; display:flex; align-items:flex-start; gap:1vh; padding:1.2vh 1.4vh; z-index:1; }
  .notification-text{ flex:1; display:flex; flex-direction:column; gap:.3vh; min-width:0; }
  .notification-title{ font-size:1.3vh; font-weight:700; letter-spacing:.015em; color:var(--text); text-shadow:0 .15vh .6vh rgba(0,0,0,.25); line-height:1.3; }
  .notification-message{ font-size:1.1vh; font-weight:400; color:var(--text-dim); line-height:1.35; word-wrap:break-word; }
  .notification-progress{ position:absolute; bottom:0; left:0; right:0; height:.3vh; background:rgba(255,255,255,.06); border-radius:0 0 .8vh .8vh; overflow:hidden; }
  .notification-progress-fill{ height:100%; background:linear-gradient(90deg, var(--notification-accent,var(--accent)) 0%, var(--notification-light,var(--accent-600)) 100%); box-shadow:0 0 .8vh var(--notification-accent,var(--accent)); animation:progress-shrink var(--progress-duration,3500ms) linear forwards; transform-origin:left; }
  @keyframes progress-shrink{ from{ transform:scaleX(1); } to{ transform:scaleX(0); } }
  .notification-info{ --notification-accent:#60a5fa; --notification-light:#93c5fd; }
  .notification-success{ --notification-accent:#34d399; --notification-light:#6ee7b7; }
  .notification-warning{ --notification-accent:#fbbf24; --notification-light:#fcd34d; }
  .notification-error{ --notification-accent:#f87171; --notification-light:#fca5a5; }
  .notification:hover{ transform:translateY(-.3vh) scale(1.015); }
  .notification:nth-child(n+2){ margin-top:-.3vh; }
...
<!-- Notifications (Premium Redesign - Compact) -->
<div class="notifications-container">
  {#each toasts as t, idx (t.id)}
    <div class="notification notification-{t.ntype}"
         style="--progress-duration: {t.timeout}ms;"
         in:fly={{ x: 300, duration: 260, easing: cubicOut, delay: idx * 40 }}
      ...
      <div class="notification-content">
        <div class="notification-text">
          ...
          {#if t.message}<div class="notification-message">{t.message}</div>{/if}
        </div>
      </div>
